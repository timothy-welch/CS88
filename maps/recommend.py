...
def group_by_centroid(restaurants, centroids):
    ...
    for x in restaurants:
        a = find_closest(restaurant_location(x), centroids)
        lst.append([a, x])
...
def find_predictor(user, restaurants, feature_fn):
    ...
    # BEGIN Question 7
    "*** YOUR CODE HERE ***"
    def sos(lst): #defining sum of squares function
        a = 0
        for i in lst:
            a += (i - mean(lst)) **2
        return a
    S_xx = sos(xs) 
    S_yy = sos(ys)
    def var(lst): #produces residual values for each number in list
        new = []
        for i in lst:
            new.append(i - mean(lst))
        return new
    xs1 = var(xs)
    ys1 = var(ys)
    S_xy = sum([xs1[i] * ys1[i] for i in range(len(xs1))]) #summation of residual products
    b = S_xy / S_xx #estimated effect
    a = mean(ys) - (b * mean(xs)) #intercept
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        if restaurant in reviewed:
            dict[restaurant_name(restaurant)] = int(user_score(user, restaurant_name(restaurant)))
        else:
